...
class SupabaseDatabase(BaseDatabase):
    ...
    async def get_events(self, user_id: Optional[str] = None, limit: int = 100, offset: int = 0) -> list[Event]:
        query = self._client.table("events").select("*").order("timestamp", desc=True).limit(limit).offset(offset)
        if user_id:
    ...
    async def get_api_keys_by_user(self, user_id: str) -> list[dict]:
        result = self._client.table("api_keys").select("*").eq("user_id", user_id).execute()
        keys = []
        for row in result.data:
            keys.append({
                "id": row["id"],
    ...
    async def get_api_key_by_hash(self, key_hash: str) -> Optional[dict]:
        result = self._client.table("api_keys").select("*").eq("key_hash", key_hash).execute()
        if not result.data:
            return None
        row = result.data[0]
        return {
    ...
    async def delete_api_key(self, key_id: str, user_id: str) -> bool:
        result = self._client.table("api_keys").delete().eq("id", key_id).eq("user_id", user_id).execute()
